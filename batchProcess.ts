...
function batchProcess(
	...
	try{
		...
		each(targets, function(idx, target){
			...
			if(passUsage > maxUsage) maxUsage = passUsage;
			...
			if(runtime.executionContext == runtime.ContextType.SCHEDULED){
					...
					}else me.percentComplete = (100*( 1 - (remainUsage/targetStartUsage)));
			}
			...
			if(!canGo) log.audit({title: 'Processing Targets', details:'processed: ' + targetCount +' targets'});
		...
		if(remainder && remainder.length){
			...
			}else log.audit({title: 'Processing Targets', details:"finished non-scheduled run " + remainder.length + " targets left to process."});
		}
	}catch(e){
